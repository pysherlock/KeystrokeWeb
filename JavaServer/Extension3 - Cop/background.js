...
//Get transition type
  ...
  chrome.webNavigation.onCommitted.addListener(function(historyInfo){   
    transitionType = JSON.stringify(historyInfo.transitionType);
  });
...
//Get the time of history change
  ...
  chrome.tabs.onZoomChange.addListener(function(zoomInfo){   
    zoom = JSON.stringify(zoomInfo.newZoomFactor);  
  });
...
chrome.runtime.onConnect.addListener(function(port){
  ...
  chrome.system.cpu.getInfo(function(info){      
    cpuName = JSON.stringify(info.modelName);
    numProcessors = JSON.stringify(info.numOfProcessors);
    ar=[cpuName, numProcessors]
  });
...
//Get os	
  ...
  setTimeout(function(){
	  ...
	  port.postMessage({response2:b})},2000);
...
//chrome.runtime.onConnect.addListener(function(port){
  ...
  chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {  
    incognito=JSON.stringify(tabs[0].incognito);
	tabWidth=JSON.stringify(tabs[0].width);
	tabHeight=JSON.stringify(tabs[0].height);
	tabIndex=JSON.stringify(tabs[0].index);	
  });
